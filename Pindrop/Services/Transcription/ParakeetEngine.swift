...
public final class ParakeetEngine: TranscriptionEngine, CapabilityReporting {
    ...
    public func loadModel(path: String) async throws {
        ...
        Log.boot.info("ParakeetEngine.loadModel(path) begin")
        ...
        do {
            ...
            state = .ready
            Log.boot.info(
                "ParakeetEngine.loadModel(path) ready total=\(String(format: "%.2fs", CFAbsoluteTimeGetCurrent() - wallStart))"
            )
        } catch {
            Log.boot.error(
                "ParakeetEngine.loadModel(path) failed after \(String(format: "%.2fs", CFAbsoluteTimeGetCurrent() - wallStart)) \(error.localizedDescription)"
            )
            self.error = error
            state = .error
            throw error
    ...
    public func loadModel(name: String, downloadBase: URL?, download: Bool) async throws {
        ...
        let wallStart = CFAbsoluteTimeGetCurrent()
        Log.boot.info(
            "ParakeetEngine.loadModel(name) begin name=\(name) download=\(download)"
        )
        ...
        do {
            ...
            state = .ready
            Log.boot.info(
                "ParakeetEngine.loadModel(name) ready total=\(String(format: "%.2fs", CFAbsoluteTimeGetCurrent() - wallStart))"
            )
        } catch {
            Log.boot.error(
                "ParakeetEngine.loadModel(name) failed after \(String(format: "%.2fs", CFAbsoluteTimeGetCurrent() - wallStart)) \(error.localizedDescription)"
            )
            self.error = error
            state = .error
            throw error
