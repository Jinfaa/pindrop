...
@MainActor
@Suite("MLXParakeetEngine (unit, offline)")
struct MLXParakeetEngineTests {
    private func makeEngine() -> MLXParakeetEngine {
        MLXParakeetEngine()
    }
    ...
    private func makeEmptyDownloadBase() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("pindrop-mlx-parakeet-engine-unit-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func transcribeWithoutModelThrows() async throws {
        ...
        } catch MLXParakeetEngine.EngineError.modelNotLoaded {
        } catch {
    ...
    @Test func transcribeEmptyAudioThrows() async throws {
        ...
        } catch MLXParakeetEngine.EngineError.invalidAudioData {
        } catch {
            Issue.record("Unexpected error: \(error)")
        }
    }
    ...
    @Test func engineErrorDescriptions() {
        #expect(MLXParakeetEngine.EngineError.modelNotLoaded.errorDescription == "Model is not loaded")
        #expect(MLXParakeetEngine.EngineError.invalidAudioData.errorDescription == "Invalid audio data")
        let message = "boom"
        #expect(
            MLXParakeetEngine.EngineError.transcriptionFailed(message).errorDescription
                == "Transcription failed: \(message)"
        )
        #expect(
            MLXParakeetEngine.EngineError.modelNotFound("x").errorDescription?
                .contains("x") == true
        )
    }
